...
function ConnectionForm({ onConnect, loading, error }: Props) {
  ...
  const handleEmailChange = async (email: string) => {
    ...
    if (email.includes("@") && email.split("@")[1]?.includes(".")) {
      ...
      setAutoConfigStatus("Suche Server-Einstellungen...");
      ...
      try {
        ...
        if (config.imap_host) {
          ...
          setAutoConfigStatus("Server-Einstellungen gefunden!");
        } else {
          setAutoConfigStatus("Verwende Standard-Einstellungen");
        }
      } catch (e) {
        setAutoConfigStatus("AutoConfig fehlgeschlagen");
    ...
    <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
      ...
      {savedAccounts.length > 0 && (
        <div className="mb-4">
          ...
          <div className="flex gap-2">
            ...
            >
              <option value="">Neues Konto...</option>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          />
          {autoConfigStatus && (
            <p className={`text-xs mt-1 ${autoConfigLoading ? "text-blue-600" : autoConfigStatus.includes("gefunden") ? "text-green-600" : "text-gray-500"}`}>
              {autoConfigLoading && (
