...
import { SavedAccount, CacheStats } from "../types/mail";
...
function AccountSettings({ onClose }: Props) {
  ...
  const loadAccounts = async () => {
    try {
      ...
      if (savedAccounts.length > 0 && !selectedAccountId) {
        // Pass the account directly since state hasn't updated yet
  ...
  const selectAccount = async (id: string, accountList?: SavedAccount[]) => {
    // Use provided accountList or fall back to state
    const searchList = accountList || accounts;
    const account = searchList.find((a) => a.id === id);
    if (account) {
      setSelectedAccountId(id);
      // Set default values for cache settings if not present
      setFormData({
        ...account,
  ...
  const handleChange = (field: keyof SavedAccount, value: string | number | boolean) => {
    if (!formData) return;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await loadAccounts();
      setMessage({ type: "success", text: "Einstellungen gespeichert. Bei Änderungen an Verbindungsdaten ist möglicherweise eine Neuverbindung erforderlich." });
    } catch (e) {
                ...
                <div className="border-t pt-4 mt-4">
                  ...
                  <div className="flex items-center mb-4">
                    ...
                    <label htmlFor="cacheEnabled" className="ml-2 block text-sm text-gray-700">
                      E-Mails lokal cachen (für Offline-Nutzung und Suche)
                    </label>
                  </div>
                    ...
                    <>
                      ...
                      <div className="flex items-center mb-3">
                        ...
                        <label htmlFor="cacheBody" className="ml-2 block text-sm text-gray-700">
                          E-Mail-Inhalt speichern (für Volltextsuche)
                        </label>
                      </div>
                      ...
                      <div className="flex items-center mb-4">
                        ...
                        <label htmlFor="cacheAttachments" className="ml-2 block text-sm text-gray-700">
                          Anhänge speichern (erhöht Speicherverbrauch)
                        </label>
                      </div>
                      ...
                      <div className="bg-gray-50 rounded p-3 mb-4">
                        ...
                        {cacheStats && cacheStats.emailCount > 0 ? (
                          ...
                          <div className="text-sm text-gray-500">
                            <p className="mb-2">Noch keine E-Mails im Cache.</p>
                            <p className="text-xs">
                              Nach dem Speichern der Einstellungen: Schließe dieses Fenster und klicke auf den
                              <span className="inline-flex items-center mx-1">
                                <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                                </svg>
                              </span>
                              Sync-Button in der Kopfzeile.
                            </p>
                          </div>
                        )}
                      </div>
